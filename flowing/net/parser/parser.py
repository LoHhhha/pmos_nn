...
class Parser:
    ...
    def __init__(
        ...
        if not os.path.isfile(network_template_path):
            raise ValueError(
                f"cannot find the network template at {network_template_path}"
            )
        with open(network_template_path, "r", encoding="utf-8") as f:
            self.network_template = "".join(f.readlines())
    ...
    def _init_layer_object(self):
        for node in self.net_nodes:
            ...
            # todo: now only ignore the content error
            node.layer_init_kwargs["ignore_content_exception"] = True

            if data_amount:
                node.layer_object = getattr(self.layers_package, node.api_name)(
                    data_amount=data_amount,
                    **node.layer_init_kwargs
                )
            else:
                node.layer_object = getattr(self.layers_package, node.api_name)(**node.layer_init_kwargs)
    ...
    def network_class(self, save_path: str = None) -> Any:
        class_str = self.network_template.format(
            time=datetime.now(),
            version=VERSION,
            feature_supported="Full" if self.dependencies_ready else "Limited",
            more_information="",
            other_import="",
        ...
        try:
            ...
            if not self.dependencies_ready:
                raise RuntimeError("dependencies isn't ready")
        ...
        if exec_success and self.dependencies_ready:
    ...
    def _push_all_layer_name(self):
        ...
        for idx in range(self.input_nodes_size):
            node = self.input_nodes[idx]
            if node.name is ...:
                node.name = Mate.INPUT_NAME_FMT.format(idx=idx)
        ...
        for idx in self._parse_sequence_index_list:
            ...
            node.layer_object.layer_name = Mate.LAYER_NAME_FMT.format(api_name=snake_case(node.api_name),
                                                                      api_idx=api_idx)
            node.layer_object.output_name = Mate.LAYER_OUTPUT_NAME_FMT.format(
        ...
        for idx in range(self.output_nodes_size):
            ...
            node.name = Mate.OUTPUT_NAME_FMT.format(idx=idx) if node.name is ... else node.name
